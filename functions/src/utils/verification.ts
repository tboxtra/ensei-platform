import { XLinkValidation, ValidationResult, ExpertiseMatch, UserExpertise, ReviewAssignment } from '../types/verification';
 ...
 */
export const validateXLink = async (link: string): Promise<XLinkValidation> => {
  try {
 ...
 */
export const validateUsernameMatch = (
  submissionLink: string,
  userXUsername: string
): ValidationResult => {
  const linkValidation = validateXLink(submissionLink);
...
export const calculateExpertiseMatch = (
  reviewer: UserExpertise,
  submission: { platform: string; missionType: string; taskType: string }
): ExpertiseMatch => {
  let matchScore = 0;
 ...
 */
export const assignReviewersByExpertise = (
  availableReviewers: UserExpertise[],
  submission: { platform: string; missionType: string; taskType: string },
  requiredReviews: number = 5
): ReviewAssignment[] => {
  // Calculate match scores for all reviewers
  const matches = availableReviewers
    .map(reviewer => calculateExpertiseMatch(reviewer, submission))
 ...
 */
export const shouldApproveSubmission = (reviews: { rating: number }[]): boolean => {
 ...
 */
export const isSubmissionExpired = (createdAt: Date): boolean => {
  const now = new Date();
 ...
 */
export const getTimeRemaining = (createdAt: Date): { hours: number; minutes: number; expired: boolean } => {
  const now = new Date();
 ...
 */
export const validateReviewSubmission = (review: {
  rating: number;
  ...
  const linkValidation = validateXLink(review.reviewerCommentLink);
  if (!linkValidation.isValidUrl || !linkValidation.isXLink) {
 ...
 */
export const generateVerificationId = (prefix: string): string => {
