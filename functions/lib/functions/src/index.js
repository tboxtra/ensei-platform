...
app.post('/v1/wallet/deposit', verifyFirebaseToken, async (req, res) => {
    try {
        const userId = req.user.uid;
...
app.put('/v1/user/profile', verifyFirebaseToken, async (req, res) => {
    try {
        ...
        const result = await (0, user_data_integrity_1.updateUserProfileSafely)(userId, {
            ...updateData,
...
const toSubmission = (id, t) => {
    ...
    const firstFrom = (name) => (name || '').trim().split(/\s+/)[0] || null;
    ...
    const nice = (s) => String(s || '').toLowerCase().replace(/^auto_/, '');
...
app.post('/v1/admin/backfill-mission-ids', verifyFirebaseToken, async (req, res) => {
    try {
        const userId = req.user.uid;
...
exports.onVerificationWrite = functions.firestore
    ...
    try {
        ...
        if (!becameVerified)
            return;
        const { uid, missionId, taskId, missionType, honorsPerTask, tasksRequired, rewardPerUser } = after;
            ...
            if (!userTaskSnap.exists) {
                ...
                if (completed && !prog.completed) {
                    ...
                    if (missionType === 'fixed' && rewardPerUser) {
                        // Update user stats
                        tx.set(userStatsRef, { totalEarned: firebaseAdmin.firestore.FieldValue.increment(rewardPerUser) }, { merge: true });
                        // Update wallet balance
...
exports.onDegenWinnersChosenV2 = degen_winner_handler_1.onDegenWinnersChosen;
exports.onDegenMissionCompleted = degen_winner_handler_1.onDegenMissionCompleted;
