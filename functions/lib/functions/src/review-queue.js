...
exports.getReviewQueue = functions.region('us-central1').https.onCall(async (data, context) => {
    ...
    for (const doc of partsSnap.docs) {
        ...
        for (const t of tasks) {
            ...
            if (excludeKeys.includes(submissionKey))
                continue;
    ...
    const open = notSkipped.filter((t, i) => {
        const s = aggSnaps[i];
        if (!s.exists)
            return true; // No aggregate yet, so it's open
        const d = s.data();
