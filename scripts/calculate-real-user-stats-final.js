        ...
        for (const [missionId, completedTasks] of userVerifiedByMission) {
          try {
            ...
            if (missionReward && missionReward.totalHonors > 0) {
              ...
              if (completedTasks.size >= requiredTasks.length && requiredTasks.length > 0) {
                totalEarned += missionReward.totalHonors;
                console.log(`   Mission ${missionId}: Awarded ${missionReward.totalHonors} honors (${completedTasks.size}/${requiredTasks.length} tasks)`);
              }
            }
          } catch (error) {
