...
export const MissionCard: React.FC<MissionCardProps> = ({ mission, onStatusChange }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6 hover:shadow-md transition-shadow">
        ...
        <div className="flex items-center space-x-2">
          ...
          {mission.status === 'active' && (
            <div className="flex space-x-1">
              <button
                onClick={() => onStatusChange(mission.id, 'paused')}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
        <div>
          ...
          <div className="text-lg font-semibold text-gray-900">
            ${mission.totalCostUsd.toLocaleString()}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="text-lg font-semibold text-gray-900">
            {mission.model === 'fixed' 
              ? `${mission.perUserHonors?.toLocaleString()} Honors`
              : `${mission.perWinnerHonors?.toLocaleString()} Honors`
            }
      ...
      <div className="flex items-center justify-between text-sm text-gray-500">
        <div>
          Created by {mission.creatorId} • {formatDate(mission.createdAt)}
        </div>
