          ...
          <ModernCard className="bg-gradient-to-r from-green-500/20 to-emerald-500/20 border-green-500/30">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-white">{balance?.honors?.toLocaleString() || '0'} Honors</p>
                <p className="text-sm text-green-300">≈ ${balance?.usd?.toFixed(2) || '0.00'} USD</p>
              </div>
          ...
          <ModernCard className="bg-gradient-to-r from-yellow-500/20 to-orange-500/20 border-yellow-500/30">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-white">{balance?.pendingHonors?.toLocaleString() || '0'} Honors</p>
                <p className="text-sm text-yellow-300">≈ ${balance?.pendingUsd?.toFixed(2) || '0.00'} USD</p>
              </div>
              <div className="text-4xl">⏳</div>
            </div>
          </ModernCard>
          ...
          <ModernCard className="bg-gradient-to-r from-blue-500/20 to-purple-500/20 border-blue-500/30">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-white">{((balance?.honors || 0) + (balance?.pendingHonors || 0)).toLocaleString()} Honors</p>
                <p className="text-sm text-blue-300">≈ ${((balance?.usd || 0) + (balance?.pendingUsd || 0)).toFixed(2)} USD</p>
              </div>
              <div className="text-4xl">📈</div>
            </div>
          </ModernCard>
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {transactions.map((transaction) => (
                <div
                  key={transaction.id}
                  className="flex items-center justify-between p-4 bg-gray-800/50 rounded-lg border border-gray-700/50 hover:bg-gray-700/50 transition-colors"
                >
                  ...
                  <div className="text-right">
                    ...
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${transaction.status === 'completed' ? 'bg-green-500/20 text-green-400' :
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-400">Complete missions to start earning Honors!</p>
            </div>
          )}
        </ModernCard>
        ...
        {showWithdrawModal && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-gray-900 rounded-2xl p-8 max-w-md w-full mx-4 border border-gray-700">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium mb-2">Amount (Honors)</label>
                  <input
                    type="number"
                ...
                {withdrawAmount && (
                  <div className="bg-gray-800/50 rounded-lg p-4">
                    <p className="text-sm text-gray-400">Estimated USD Value:</p>
