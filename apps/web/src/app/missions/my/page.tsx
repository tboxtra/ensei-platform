...
export default function MyMissionsPage() {
    ...
    // While redirecting, render nothing (no spinner overlay)
    if (!authReady) return null;

    // Past this point the page must render regardless of queries
    return <MyMissionsContent uid={userRef.current?.uid ?? ''} />;
}

function MyMissionsContent({ uid }: { uid: string }) {
    const router = useRouter();
    ...
    useEffect(() => {
        // Refetch once uid is available ensures fresh data after any route
        if (uid) {
            refetch();
        }
    }, [uid, refetch]);
