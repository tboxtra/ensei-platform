...
// Robust "verified clicks" getter (uses whatever field exists, or derives)
function getVerifiedClicks(m: any): number {
  return (
    m.verified_clicks ??
    m.verified_count ??
    m.verifications_count ??
    (Array.isArray(m.submissions)
      ? m.submissions.filter((s: any) => s?.status === 'verified').reduce((sum: number, s: any) => {
          // if submission has multiple verified tasks, count them
          const vt = s?.verified_tasks ?? s?.verifiedTasks ?? 1; // fallback to 1 per verified submission
          return sum + (Number(vt) || 1);
        }, 0)
      : 0)
  ) || 0;
}
            ...
            {filteredMissions.map((m) => (
              <MissionListItem
                ...
                mission={{
                  ...
                  __verifiedClicks: getVerifiedClicks(m),
                }}
                dense
                onRefetch={refetch}
              />
